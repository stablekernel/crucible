...
A restored instance re-arms the configuration's effects — pending `after` timers and in-flight actors — so it resumes exactly where it was captured, as a recovering host would after a process restart.
