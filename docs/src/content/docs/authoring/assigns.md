...
Keep assigns and actions distinct. An **assign** changes the machine's own context. An **action** (`Do`) emits an effect for the host to perform IO. A reducer must stay total and side-effect free, with no IO, no errors, and no clock reads, because the engine may replay it during recovery or analysis. If you need the outside world to react, return an effect from a `Do`; if you need the data to advance, fold it in an `Assign`.
