...
To check that two machines agree — a reference and a refactored subject, or a Go-authored machine versus its JSON round-trip — `CompareMachines` runs a scenario set through both and reports field-level `Mismatch`es as an `*ErrConformance`:
