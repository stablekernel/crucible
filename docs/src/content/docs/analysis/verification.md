...
- **`ReachAvoiding(target, avoid...)`** — *safety*: reach `target` along a run that never touches the avoided states. Read with `result.ConditionalReach(target)`.
- **`AlwaysEventually(target)`** — *liveness*: from every reachable configuration, can `target` still be reached? On violation, `result.Liveness(target)` names the stuck configuration.
- **`CheckInvariant(MutualExclusion / Implies / NeverActive, ...)`** — configuration invariants. `result.Invariant(label)` returns the counterexample configuration when one is reachable.
