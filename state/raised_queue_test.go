...
// r2 queued. A clean kernel discards the residual queue when the macrostep
// settles (success or error), so a subsequent Fire("x") that moves s3 -> s4 must
// NOT then replay the stale r2 (which would otherwise drive s4 -> s5).
